...
const ItemDetail = ({ item }) => {
  ...
  const { addItem, stockFree, itemStock } = useContext(CartContext);
  console.log('zz', unitsChoosed);

  useEffect(()=>{
    itemStock(item.stock, unitsChoosed);
  },[unitsChoosed]);
  ...
  return (
    <div key={item.id} className='col rounded m-3'>
      ...
      <ItemCount stock={stockFree} initial={1} onAdd={(n) => { setUnitsChoosed(n); addItem({ ...item, quantity: n }) }} />
    </div>
